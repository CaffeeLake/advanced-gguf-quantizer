...
static constexpr double SELECTOR_LOGITS_FALLBACK_GIB = 4.0;
static constexpr double SELECTOR_LOGITS_RESERVE_MIN_GIB = 4.0;
static constexpr double SELECTOR_LOGITS_RESERVE_MAX_GIB = 16.0;
static constexpr double SELECTOR_LOGITS_RESERVE_FRACTION = 0.20;
static constexpr int64_t SELECTOR_PROGRESS_HEARTBEAT_SEC = 30;

static std::atomic<int32_t> g_selector_kld_threads_override{0};
static std::atomic_bool g_selector_skip_requested{false};
static std::string g_selector_skip_file;
...
int32_t selector_kld_threads_override() {
    return g_selector_kld_threads_override.load(std::memory_order_acquire);
}

void selector_set_kld_threads_override(int32_t threads) {
    g_selector_kld_threads_override.store(threads, std::memory_order_release);
}

void selector_set_skip_state(std::string skip_file, bool skip_requested) {
    g_selector_skip_file = std::move(skip_file);
    g_selector_skip_requested.store(skip_requested, std::memory_order_release);
}

bool selector_skip_requested(const char * phase) {
    if (g_selector_skip_requested.load(std::memory_order_acquire)) {
        return true;
    }
    if (g_selector_skip_file.empty()) {
        return false;
    }
    std::error_code ec;
    if (!std::filesystem::exists(g_selector_skip_file, ec)) {
        return false;
    }
    bool expected = false;
    if (g_selector_skip_requested.compare_exchange_strong(
            expected, true, std::memory_order_acq_rel)) {
        fprintf(stderr,
            "selector: skip remaining tuning requested%s%s via %s; using best available policy after current phase\n",
            phase != nullptr && phase[0] != '\0' ? " during " : "",
            phase != nullptr && phase[0] != '\0' ? phase : "",
            g_selector_skip_file.c_str());
    }
    return true;
}
...
selector_logits_budget selector_default_logits_budget() {
    selector_logits_budget budget;
    budget.available_gib = selector_host_mem_available_gib();
    if (budget.available_gib <= 0.0 || !std::isfinite(budget.available_gib)) {
        budget.max_gib = SELECTOR_LOGITS_FALLBACK_GIB;
        return budget;
    }

    budget.reserve_gib = std::min(
        SELECTOR_LOGITS_RESERVE_MAX_GIB,
        std::max(SELECTOR_LOGITS_RESERVE_MIN_GIB,
            budget.available_gib * SELECTOR_LOGITS_RESERVE_FRACTION));
    budget.max_gib = std::max(SELECTOR_LOGITS_MIN_GIB, budget.available_gib - budget.reserve_gib);
    return budget;
}

selector_progress_heartbeat::selector_progress_heartbeat(std::string label, int64_t total) :
...
selector_progress_heartbeat::~selector_progress_heartbeat() {
    stop();
}

void selector_progress_heartbeat::update(int64_t completed, int64_t total, std::string detail, bool print_now) {
    ...
    if (print_now || now - last_print_ >= std::chrono::seconds(SELECTOR_PROGRESS_HEARTBEAT_SEC)) {
        print_locked(now);
    }
}

void selector_progress_heartbeat::update(int64_t completed, std::string detail, bool print_now) {
    update(completed, -1, std::move(detail), print_now);
}

void selector_progress_heartbeat::detail(std::string detail, bool print_now) {
    std::lock_guard<std::mutex> lock(mu_);
    if (!detail.empty()) {
        detail_ = std::move(detail);
    }
...
void selector_progress_heartbeat::eta_hint(std::string eta, bool print_now) {
    std::lock_guard<std::mutex> lock(mu_);
...
void selector_progress_heartbeat::finish(std::string detail) {
    {
        std::lock_guard<std::mutex> lock(mu_);
        if (!detail.empty()) {
            detail_ = std::move(detail);
...
void selector_progress_heartbeat::stop() {
    {
        std::lock_guard<std::mutex> lock(mu_);
        if (stopping_) {
            return;
...
void selector_progress_heartbeat::run() {
    ...
    while (!stopping_) {
        ...
        const auto now = std::chrono::steady_clock::now();
        if (now - last_print_ >= std::chrono::seconds(SELECTOR_PROGRESS_HEARTBEAT_SEC)) {
            print_locked(now);
        }
    }
}

void selector_progress_heartbeat::print_locked(std::chrono::steady_clock::time_point now) {
    ...
    if (total_ > 0 && completed_ > 0 && completed_ < total_ && eta_hint_.empty()) {
        eta = selector_format_duration(elapsed_s * (double) (total_ - completed_) / (double) completed_);
    ...
    if (total_ > 0) {
        ...
        fprintf(stderr,
            ...
            total_,
            pct,
            selector_format_duration(elapsed_s).c_str(),
            eta.c_str(),
            detail_.empty() ? "" : " ",

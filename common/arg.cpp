...
static bool common_params_parse_ex(int argc, char ** argv, common_params_context & ctx_arg) {
    ...
    // setup log directly from params.verbosity for retained frontends
    ...
    // parse command line and environment values
    parse_cli_args();

    // export_graph_ops loads only metadata
    const bool skip_model_resolution = ctx_arg.ex == LLAMA_EXAMPLE_EXPORT_GRAPH_OPS;
    ...
    // handle local model helper params
    if (!skip_model_resolution) {
        common_params_handle_models(params, ctx_arg.ex);
    }

    // model is required (except for server)
    // TODO @ngxson : maybe show a list of available models in CLI in this case
    if (params.model.path.empty() && ctx_arg.ex != LLAMA_EXAMPLE_SERVER && !skip_model_resolution && !params.usage && !params.completion) {
...
static bool common_example_shows_sampling(llama_example ex) {
    switch (ex) {
        case LLAMA_EXAMPLE_IMATRIX:
        case LLAMA_EXAMPLE_PERPLEXITY:
        case LLAMA_EXAMPLE_BENCH:
        case LLAMA_EXAMPLE_FIT_PARAMS:
            return false;
        default:
            return true;
    }
...
static void common_params_print_usage(common_params_context & ctx_arg) {
    ...
    if (common_example_shows_sampling(ctx_arg.ex)) {
        printf("\n\n----- sampling params -----\n\n");
...
static void common_params_print_completion(common_params_context & ctx_arg, const char * argv0) {
    std::vector<common_arg *> common_options;
    std::vector<common_arg *> sampling_options;
    std::vector<common_arg *> spec_options;
    std::vector<common_arg *> specific_options;
    ...
    printf("_advanced_gguf_quantizer_completions() {\n");
    ...
    std::string executable = argv0 != nullptr ? argv0 : "";
    const size_t slash = executable.find_last_of("/\\");
    if (slash != std::string::npos) {
        executable = executable.substr(slash + 1);
    }
    if (!executable.empty()) {
        printf("complete -F _advanced_gguf_quantizer_completions %s\n", executable.c_str());
    }
}
...
bool common_params_parse(int argc, char ** argv, common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    try {
        ...
        if (ctx_arg.params.completion) {
            common_params_print_completion(ctx_arg, argc > 0 ? argv[0] : nullptr);
            exit(0);
        }
        params.lr.init();
...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--completion-bash"},
        "print source-able bash completion script for this executable",
        [](common_params & params) {
    ...
    add_opt(common_arg(
        ...
        "use a single unified KV buffer shared across parallel sequences (default: enabled when --parallel is auto)",
        [](common_params & params, bool value) {
            params.kv_unified = value;
        }
    ).set_env("LLAMA_ARG_KV_UNIFIED").set_examples({LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_PERPLEXITY, LLAMA_EXAMPLE_BENCH, LLAMA_EXAMPLE_PARALLEL}));
    add_opt(common_arg(
        {"--cache-idle-slots"},
        {"--no-cache-idle-slots"},
    ...
    add_opt(common_arg(
        ...
        "JSON schema to constrain generations (https://json-schema.org/), e.g. `{}` for any JSON object\nExternal $refs are not resolved by the retained C++ converter; use --grammar/--grammar-file for preconverted grammars",
        [](common_params & params, const std::string & value) {
    ...
    add_opt(common_arg(
        {"--save-all-logits-resume"},
        "resume/appending --save-all-logits output after the complete chunks already present in the logits file",
        [](common_params & params) {
            params.logits_file_resume = true;
        }
    ).set_examples({LLAMA_EXAMPLE_PERPLEXITY}));
    add_opt(common_arg(
        {"--ppl-stride"}, "N",
    ...
    add_opt(common_arg(
        {"-mm", "--mmproj"}, "FILE",
        "path to a multimodal projector file for legacy upstream-compatible multimodal params",
        [](common_params & params, const std::string & value) {
    ...
    add_opt(common_arg(
        ...
        string_format("whether to use multimodal projector file (if provided) (default: %s)", params.no_mmproj ? "disabled" : "enabled"),
        [](common_params & params, bool value) {

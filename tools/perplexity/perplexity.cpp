...
static std::pair<double, float> log_softmax(int n_vocab, const float * logits, const uint16_t * base_log_prob, int tok, kl_divergence_result & kld) {
    ...
    const float raw_max_logit = max_logit;
    double sum_exp = 0.0;
    double sum_exp_shifted_logit = 0.0;
    for (int i = 0; i < n_vocab; ++i) {
        const double exp_shifted = expf(logits[i] - max_logit);
        sum_exp += exp_shifted;
        sum_exp_shifted_logit += exp_shifted * (double) (logits[i] - max_logit);
    }
    const float log_sum_exp = log(sum_exp);
    ...
    const double q_prob_base_top = exp((double) logits[imax_base] - (double) raw_max_logit - log_sum_exp);
    const double base_top_prob = exp((double) p_log_base_max) / base_norm;
    const double base_prob_q_top = exp((double) (scale * base_log_prob[imax] + min_log_prob)) / base_norm;
    const double top_prob_diff = q_prob_base_top - base_top_prob;
    const double top_flip_weight = imax == imax_base ? 0.0 : std::max(0.0, base_top_prob - base_prob_q_top);
    kld.sum_entropy_diff2 += entropy_diff * entropy_diff;
    kld.sum_top_prob_diff2 += top_prob_diff * top_prob_diff;
    kld.sum_top_flip_weight += top_flip_weight;
    ...
    auto compute = [&mutex, &counter, &base_log_probs, &kld, n_vocab, logits, tokens, n_token, nv, kld_values, p_diff_values] () {
        ...
        while (true) {
            ...
            if (i >= n_token) {
                ...
                kld.sum_entropy_diff2 += local_kld.sum_entropy_diff2;
                kld.sum_top_prob_diff2 += local_kld.sum_top_prob_diff2;
                kld.sum_top_flip_weight += local_kld.sum_top_flip_weight;
...
static results_perplexity perplexity_v2(llama_context * ctx, const common_params & params) {
    ...
    // Run `./perplexity -m /models/model.gguf -f wiki.test.raw`
...
static results_perplexity perplexity(llama_context * ctx, const common_params & params, const int32_t n_ctx) {
    ...
    // Download: https://huggingface.co/datasets/ggml-org/ci/resolve/main/wikitext-2-raw-v1.zip
    // Run `./llama-perplexity -m /models/model.gguf -f wiki.test.raw`
    // Output: `perplexity: 13.5106 [114/114]`
    // BOS tokens will be added for each chunk before eval
    ...
    if (!params.logits_file.empty()) {
        const int nv = 2*((n_vocab + 1)/2) + 4;
        const uint64_t header_bytes = 8 + sizeof(uint32_t) + sizeof(n_vocab) + sizeof(n_chunk);
        const uint64_t token_bytes = (uint64_t) n_chunk * (uint64_t) n_ctx * sizeof(tokens[0]);
        const uint64_t logp_chunk_bytes = (uint64_t) (n_ctx - 1 - n_ctx/2) * (uint64_t) nv * sizeof(uint16_t);

        if (params.logits_file_resume) {
            std::ifstream in(params.logits_file.c_str(), std::ios::binary);
            if (!in.is_open()) {
                LOG_ERR("%s: failed to open %s for resume\n", __func__, params.logits_file.c_str());
                return {};
            }

            char magic[8];
            uint32_t file_n_ctx = 0;
            int file_n_vocab = 0;
            int file_n_chunk = 0;
            in.read(magic, sizeof(magic));
            in.read(reinterpret_cast<char *>(&file_n_ctx), sizeof(file_n_ctx));
            in.read(reinterpret_cast<char *>(&file_n_vocab), sizeof(file_n_vocab));
            in.read(reinterpret_cast<char *>(&file_n_chunk), sizeof(file_n_chunk));
            if (!in || std::memcmp(magic, "_logits_", sizeof(magic)) != 0) {
                LOG_ERR("%s: %s does not look like a logits file\n", __func__, params.logits_file.c_str());
                return {};
            }
            if ((int) file_n_ctx != n_ctx || file_n_vocab != n_vocab || file_n_chunk != n_chunk) {
                LOG_ERR("%s: cannot resume %s: header has ctx=%u vocab=%d chunks=%d, current run has ctx=%d vocab=%d chunks=%d\n",
                        __func__, params.logits_file.c_str(), file_n_ctx, file_n_vocab, file_n_chunk, n_ctx, n_vocab, n_chunk);
                return {};
            }

            std::vector<llama_token> file_tokens((size_t) n_chunk * n_ctx);
            in.read(reinterpret_cast<char *>(file_tokens.data()), (std::streamsize) token_bytes);
            if (!in) {
                LOG_ERR("%s: cannot resume %s: token table is incomplete\n", __func__, params.logits_file.c_str());
                return {};
            }
            if (!std::equal(file_tokens.begin(), file_tokens.end(), tokens.begin())) {
                LOG_ERR("%s: cannot resume %s: tokenized input does not match the existing logits token table\n",
                        __func__, params.logits_file.c_str());
                return {};
            }
            in.close();
            ...
            std::error_code ec;
            const uint64_t file_size = (uint64_t) std::filesystem::file_size(params.logits_file, ec);
            if (ec) {
                LOG_ERR("%s: cannot stat %s: %s\n", __func__, params.logits_file.c_str(), ec.message().c_str());
                return {};
            }
            const uint64_t logp_start = header_bytes + token_bytes;
            if (file_size < logp_start) {
                LOG_ERR("%s: cannot resume %s: file is smaller than the expected header+tokens region\n",
                        __func__, params.logits_file.c_str());
                return {};
            }

            const uint64_t complete_chunks = std::min<uint64_t>((uint64_t) n_chunk, (file_size - logp_start) / logp_chunk_bytes);
            const uint64_t aligned_size = logp_start + complete_chunks * logp_chunk_bytes;
            if (aligned_size != file_size) {
                std::filesystem::resize_file(params.logits_file, aligned_size, ec);
                if (ec) {
                    LOG_ERR("%s: failed to trim partial logits chunk in %s: %s\n",
                            __func__, params.logits_file.c_str(), ec.message().c_str());
                    return {};
                }
                LOG_WRN("%s: trimmed partial logits chunk in %s from %" PRIu64 " to %" PRIu64 " bytes\n",
                        __func__, params.logits_file.c_str(), file_size, aligned_size);
            }

            chunk_begin = (int) complete_chunks;
            if (chunk_begin >= n_chunk) {
                LOG_INF("%s: logits file %s already has all %d chunks\n", __func__, params.logits_file.c_str(), n_chunk);
                return {std::move(tokens), 0.0, std::move(logit_history), std::move(prob_history)};
            }

            logits_stream.open(params.logits_file.c_str(), std::ios::binary | std::ios::app);
            if (!logits_stream.is_open()) {
                LOG_ERR("%s: failed to open %s for appending\n", __func__, params.logits_file.c_str());
                return {};
            }
            LOG_INF("%s: resuming logits file %s from chunk %d/%d\n", __func__, params.logits_file.c_str(), chunk_begin, n_chunk);
        } else {
    ...
    for (int i = chunk_begin; i < n_chunk; i += n_seq) {
        ...
        if (i == chunk_begin) {
            ...
            const float t_total = std::chrono::duration<float>(t_end - t_start).count();
            LOG_INF("%s: %.2f seconds per pass - ETA ", __func__, t_total);
            int total_seconds = (int)(t_total*(n_chunk - chunk_begin)/n_seq);
            if (total_seconds >= 60*60) {
                LOG("%d hours ", total_seconds / (60*60));
                total_seconds = total_seconds % (60*60);
            }
...
static void kl_divergence(llama_context * ctx, const common_params & params) {
    ...
    LOG("Top flip weight: %10.6lf\n", kld.sum_top_flip_weight/kld.count);
    LOG("Top prob RMSE  : %10.6lf\n", sqrt(std::max(0.0, kld.sum_top_prob_diff2/kld.count)));
    LOG("Entropy RMSE   : %10.6lf\n", sqrt(std::max(0.0, kld.sum_entropy_diff2/kld.count)));
}

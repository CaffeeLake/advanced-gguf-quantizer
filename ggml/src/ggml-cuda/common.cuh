...
#define QR_FP8 1
#define QI_FP8 (QK_FP8 / (4 * QR_FP8))

struct block_fp8 {
    uint8_t qs[QK_FP8_FRAGS][QK_FP8_SUB];
...
static inline GGML_HD uint32_t ggml_cuda_fp8_get4_u8containers(const block_fp8 & block, int frag, int word) {
    const uint8_t * p = &block.qs[frag][4 * word];
...
static inline GGML_HD void ggml_cuda_fp8_set4_u8containers(block_fp8 & block, int frag, int word, uint32_t packed4) {
    uint8_t * p = &block.qs[frag][4 * word];
    p[0] = (uint8_t) (packed4 >>  0);
    p[1] = (uint8_t) (packed4 >>  8);
    p[2] = (uint8_t) (packed4 >> 16);
...
static inline bool ggml_cuda_mxfp6_disable_q8_to_fp8_mmvq() {
    const char * mode = getenv("GGML_CUDA_MXFP6_MMVQ");
    return mode != nullptr &&
        (strcmp(mode, "no-q8-fp8") == 0 || strcmp(mode, "direct-q8") == 0 || strcmp(mode, "q8") == 0 ||
         strcmp(mode, "fallback") == 0);
}
...
static __device__ __forceinline__ float ggml_cuda_nvfp4_scale_to_fp32_half(uint8_t x) {
    return ggml_cuda_ue4m3_to_fp32(x);
}

static __device__ __forceinline__ float ggml_cuda_e4m3_to_fp32(uint8_t x) {
#if defined(FP8_AVAILABLE) && !defined(GGML_USE_HIP)
    const uint32_t bits = x * ((x & 0x7F) != 0x7F); // Convert NaN to 0.0f to match CPU implementation.
    const __nv_fp8_e4m3 xf = *reinterpret_cast<const __nv_fp8_e4m3 *>(&bits);
    ...
    const int exp = (abs_code >> 3) & 0xF;
    const int man = abs_code & 0x7;

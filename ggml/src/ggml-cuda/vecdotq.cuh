...
static __device__ __forceinline__ int mxfp6_e2m3_pack4_i8x8(const uint32_t packed4) {
    ...
    const uint32_t c2 = (packed4 >> 16) & 0x3f;
    const uint32_t c3 = (packed4 >> 24) & 0x3f;
...
static __device__ __forceinline__ float vec_dot_mxfp6_e2m3_q8_1_bw(
    const void * __restrict__ vbq, const block_q8_1 * __restrict__ bq8_1, const int & kbx, const int & iqs,
    const uint32_t channel_x) {

    static_assert(VDR_MXFP6_E2M3_Q8_1_MMVQ <= 8, "MXFP6_E2M3 MMVQ VDR must stay within one K32 scale fragment.");

    const uint32_t packed_kbx = (uint32_t) kbx;
    const int row_in_tile = packed_kbx >> 28;
    const uint32_t block_rel = packed_kbx & 0x0FFFFFFFu;
    const tile_mxfp6_e2m3_blackwell_tensor * tensor = (const tile_mxfp6_e2m3_blackwell_tensor *) vbq;
    const tile_mxfp6_e2m3_blackwell & tile = tensor->tiles[block_rel];
    ...
    const float d = tensor_scale * ggml_cuda_e8m0_to_fp32(ggml_cuda_mxfp6_e2m3_bw_tile_scale(tile, row_in_tile, frag)) *
...
#pragma unroll
    for (int l = 0; l < VDR_MXFP6_E2M3_Q8_1_MMVQ; ++l) {
        ...
        const int x4 = mxfp6_e2m3_pack4_i8x8(ggml_cuda_mxfp6_e2m3_bw_tile_q_word(tile, row_in_tile, frag, word));
...
#define VDR_FP8_Q8_1_MMVQ 4
#define VDR_FP8_Q8_1_MMQ  8

static __device__ __forceinline__ int64_t fp8_dot4_i32_x512(const uint32_t x4, const uint32_t y4) {
    int64_t sumi = 0;
#pragma unroll
    for (int i = 0; i < 4; ++i) {
        const uint8_t xcode = (x4 >> (8 * i)) & 0xFF;
        const uint8_t ycode = (y4 >> (8 * i)) & 0xFF;
        sumi += (int64_t) ggml_cuda_e4m3_to_i32_x512(xcode) * (int64_t) ggml_cuda_e4m3_to_i32_x512(ycode);
    }
    return sumi;
}

static __device__ __forceinline__ int64_t mxfp6_e2m3_fp8_dot4_i32_x4096(const uint32_t x6_4, const uint32_t y8_4) {
    const int x4 = mxfp6_e2m3_pack4_i8x8(x6_4);
    int64_t sumi = 0;
#pragma unroll
    for (int i = 0; i < 4; ++i) {
        const int8_t  xv = (int8_t)  ((x4   >> (8 * i)) & 0xFF);
        const uint8_t yc = (uint8_t) ((y8_4 >> (8 * i)) & 0xFF);
        sumi += (int64_t) xv * (int64_t) ggml_cuda_e4m3_to_i32_x512(yc);
    }
    return sumi;
}

static __device__ __forceinline__ float vec_dot_fp8_q8_1(
    const void * __restrict__ vbq, const block_q8_1 * __restrict__ bq8_1, const int & kbx, const int & iqs) {

    const block_fp8 * bq8x = (const block_fp8 *) vbq + kbx;

    float sum = 0.0f;
#pragma unroll
    for (int l = 0; l < VDR_FP8_Q8_1_MMVQ; ++l) {
        const int qword = iqs + l;
        const int frag  = qword >> 3;
        const int word  = qword & 7;

        const uint32_t x4 = ggml_cuda_fp8_get4_u8containers(*bq8x, frag, word);
        const uint32_t y4 = get_int_b4(bq8_1[frag].qs, word);
        int64_t sumi = 0;
#pragma unroll
        for (int i = 0; i < 4; ++i) {
            const uint8_t xcode = (x4 >> (8 * i)) & 0xFF;
            const int8_t yq = (int8_t) ((y4 >> (8 * i)) & 0xFF);
            sumi += (int64_t) ggml_cuda_e4m3_to_i32_x512(xcode) * (int64_t) yq;
        }

        const float d = ggml_cuda_e8m0_to_fp32(bq8x->e[frag]) * __low2float(bq8_1[frag].ds) * (1.0f / 512.0f);
        sum += d * float(sumi);
    }
...
static __device__ __forceinline__ float vec_dot_fp8_fp8_mmvq(
    const void * __restrict__ vbq, const block_fp8 * __restrict__ bfp8, const int & kbx, const int & iqs) {

    const block_fp8 * bq8x = (const block_fp8 *) vbq + kbx;
    const block_fp8 * bq8y = bfp8 + kbx;
...
#pragma unroll
    for (int l = 0; l < VDR_FP8_Q8_1_MMVQ; ++l) {
        ...
        const uint32_t x4 = ggml_cuda_fp8_get4_u8containers(*bq8x, frag, word);
        const uint32_t y4 = ggml_cuda_fp8_get4_u8containers(*bq8y, frag, word);
        float sumf = 0.0f;
#pragma unroll
        for (int i = 0; i < 4; ++i) {
            const uint8_t xcode = (x4 >> (8 * i)) & 0xFF;
            const uint8_t ycode = (y4 >> (8 * i)) & 0xFF;
            sumf += ggml_cuda_e4m3_to_fp32(xcode) * ggml_cuda_e4m3_to_fp32(ycode);
        }
        const float d = ggml_cuda_e8m0_to_fp32(bq8x->e[frag]) * ggml_cuda_e8m0_to_fp32(bq8y->e[frag]);
        sum += d * sumf;
    }
...
#if defined(BLACKWELL_MMA_AVAILABLE)
static __device__ __forceinline__ float vec_dot_mxfp6_e2m3_fp8_mmvq_bw(
    const void * __restrict__ vbq, const block_fp8 * __restrict__ bfp8, const int & kbx, const int & iqs,
    const uint32_t channel_x, const int y_frag) {
...
#pragma unroll
    for (int l = 0; l < VDR_MXFP6_E2M3_Q8_1_MMVQ; ++l) {
        ...
        const uint32_t x4 = ggml_cuda_mxfp6_e2m3_bw_tile_q_word(tile, row_in_tile, frag, word);
        const uint32_t y4 = ggml_cuda_fp8_get4_u8containers(*bfp8, y_frag, word);
        const float d = tensor_scale * ggml_cuda_e8m0_to_fp32(ggml_cuda_mxfp6_e2m3_bw_tile_scale(tile, row_in_tile, frag)) *
            ggml_cuda_e8m0_to_fp32(bfp8->e[y_frag]) * (1.0f / (8.0f * 512.0f));
        sum += d * float(mxfp6_e2m3_fp8_dot4_i32_x4096(x4, y4));
    }

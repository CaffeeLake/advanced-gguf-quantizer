...
static __device__ __forceinline__ void dequantize_mxfp6(const void * vx, const int64_t ib, const int iqs, float2 & v) {
    const compact_mxfp6_k32 * x = (const compact_mxfp6_k32 *) vx;

    const int frag = iqs / QK_MXFP6_E2M3_SUB;
    const int rem0 = iqs - frag * QK_MXFP6_E2M3_SUB;
    const int rem1 = rem0 + 1;

    const uint8_t code0 = ggml_cuda_mxfp6_e2m3_get_code(x[ib], frag, rem0);
    const uint8_t code1 = ggml_cuda_mxfp6_e2m3_get_code(x[ib], frag, rem1);
    const float d = ggml_cuda_e8m0_to_fp32(x[ib].e[frag]) * (1.0f / 8.0f);

    v.x = d * float(kvalues_mxfp6_e2m3[code0]);
    v.y = d * float(kvalues_mxfp6_e2m3[code1]);
}

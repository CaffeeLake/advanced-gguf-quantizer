...
struct ggml_cuda_nvfp4_set_tls {
    void * compact = nullptr;
    size_t compact_cap = 0;

    ~ggml_cuda_nvfp4_set_tls() {
        if (compact != nullptr) {
            cudaFree(compact);
        }
    }
...
static thread_local ggml_cuda_nvfp4_set_tls g_ggml_cuda_nvfp4_set_tls;

static bool ggml_cuda_nvfp4_set_ensure_buf(void ** ptr, size_t * cap, size_t need) {
    if (need == 0 || *cap >= need) {
        return true;
    }
    if (*ptr != nullptr) {
...
static bool ggml_cuda_set_tensor_nvfp4_gpu_repack(ggml_tensor * tensor, const void * data, size_t size) {
    ...
    auto & tls = g_ggml_cuda_nvfp4_set_tls;
    if (!ggml_cuda_nvfp4_set_ensure_buf(&tls.compact, &tls.compact_cap, size)) {
        return false;
    }

    block_nvfp4_blackwell_tensor header;
    ggml_cuda_nvfp4_set_tensor_header(tensor, &header);
    cudaStream_t stream = cudaStreamPerThread;
    CUDA_CHECK(cudaMemcpyAsync(tensor->data, &header, sizeof(header), cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(cudaMemcpyAsync(tls.compact, data, size, cudaMemcpyHostToDevice, stream));

    const int64_t ne0 = tensor->ne[0];
    const int64_t nrows = tensor->ne[1];
    const int64_t nplanes = tensor->ne[2] * tensor->ne[3];
    const int64_t nblocks =
        nplanes * ggml_cuda_bw_div_up(nrows, 16) * ggml_cuda_nvfp4_blocks_per_row(ne0);
    block_nvfp4_blackwell * dst_tiles =
        reinterpret_cast<block_nvfp4_blackwell *>((char *) tensor->data + sizeof(block_nvfp4_blackwell_tensor));
    ggml_cuda_repack_tensor_nvfp4_kernel<<<(unsigned int) nblocks, 256, 0, stream>>>(
        reinterpret_cast<const uint8_t *>(tls.compact), dst_tiles, ne0, nrows, nplanes);
...
static bool ggml_cuda_set_tensor_nvfp4(ggml_tensor * tensor, const void * data, size_t offset, size_t size) {
    ...
    const size_t packed_size = ggml_cuda_nvfp4_tensor_alloc_size(tensor);
    const bool set_full_tensor = offset == 0 && size == logical_size;
    if (set_full_tensor && ggml_cuda_set_tensor_nvfp4_gpu_repack(tensor, data, size)) {
        return true;
    }
    char * buf = (char *) malloc(set_full_tensor ? packed_size : logical_size + packed_size);
    GGML_ASSERT(buf != nullptr);
    char * rows   = set_full_tensor ? nullptr : buf;
    char * packed = set_full_tensor ? buf : buf + logical_size;
    if (!set_full_tensor) {
        CUDA_CHECK(cudaMemcpyAsync(packed, tensor->data, packed_size, cudaMemcpyDeviceToHost, cudaStreamPerThread));
        CUDA_CHECK(cudaStreamSynchronize(cudaStreamPerThread));
    ...
    CUDA_CHECK(cudaMemcpyAsync(tensor->data, packed, packed_size, cudaMemcpyHostToDevice, cudaStreamPerThread));
    CUDA_CHECK(cudaStreamSynchronize(cudaStreamPerThread));
    free(buf);
    return true;
}
...
static bool ggml_cuda_set_tensor_mxfp6(ggml_tensor * tensor, const void * data, size_t offset, size_t size, int device) {
    ...
    if (storage_offset == 0 && offset == 0 && size >= MXFP6_HEADER_OFFSET) {
        tile_mxfp6_e2m3_blackwell_tensor header;
...
static bool ggml_cuda_get_tensor_nvfp4(const ggml_tensor * tensor, void * data, size_t offset, size_t size) {
    ...
    const size_t logical_size = ggml_nbytes(tensor);
    const size_t packed_size = ggml_cuda_nvfp4_tensor_alloc_size(tensor);
    char * buf = (char *) malloc(logical_size + packed_size);
    GGML_ASSERT(buf != nullptr);
    char * packed = buf;
    char * rows   = buf + packed_size;
    CUDA_CHECK(cudaMemcpyAsync(packed, tensor->data, packed_size, cudaMemcpyDeviceToHost, cudaStreamPerThread));
    CUDA_CHECK(cudaStreamSynchronize(cudaStreamPerThread));
...
static bool ggml_cuda_get_tensor_mxfp6(const ggml_tensor * tensor, void * data, size_t offset, size_t size, int device) {
    ...
    CUDA_CHECK(cudaMemcpyAsync(data, (const char *) storage->data + storage_offset + offset, size, cudaMemcpyDeviceToHost, cudaStreamPerThread));
    CUDA_CHECK(cudaStreamSynchronize(cudaStreamPerThread));
    return true;
}
...
#if defined(BLACKWELL_MMA_AVAILABLE)
        const bool use_nvfp4_layout = tensor->type == GGML_TYPE_NVFP4;
        const bool use_mxfp6_layout = ggml_cuda_use_mxfp6_blackwell_layout(tensor, id);
        if (use_nvfp4_layout) {
            size = ggml_cuda_nvfp4_tensor_size(ne0, nrows_split, /*nplanes=*/1);
            original_size = size;
        } else if (use_mxfp6_layout) {
            size = ggml_cuda_mxfp6_e2m3_tensor_alloc_size(ne0, nrows_split, /*nplanes=*/1);
            original_size = size;
        }
#else
        const bool use_nvfp4_layout = false;
        const bool use_mxfp6_layout = false;
#endif // defined(BLACKWELL_MMA_AVAILABLE)
        // pad last row to a multiple of 512 elements to avoid out-of-bounds memory accesses
        if (!use_nvfp4_layout && !use_mxfp6_layout && ne0 % MATRIX_ROW_PADDING != 0) {
            size += ggml_row_size(tensor->type, MATRIX_ROW_PADDING - ne0 % MATRIX_ROW_PADDING);
        }
...
static void ggml_backend_cuda_split_buffer_set_tensor(ggml_backend_buffer_t buffer, ggml_tensor * tensor, const void * data, size_t offset, size_t size) {
    ...
    for (int id = 0; id < ggml_backend_cuda_get_device_count(); ++id) {
        ...
        const int64_t nrows_copy = row_high - row_low;
        if (nrows_copy == 0) {
            continue;
        }

        const size_t packed_size = ggml_cuda_nvfp4_tensor_size(ne0, nrows_copy, /*nplanes=*/1);
        ...
        ggml_cuda_nvfp4_set_tensor_header(tensor, packed_tensor);
        ggml_cuda_repack_tiles_nvfp4(ne0, nrows_copy, host_rows + row_low * row_size, packed_tensor->tiles);
    ...
    const char * host_rows = (const char *) data + MXFP6_HEADER_OFFSET;
    bool handled = false;
    ...
    for (int id = 0; id < ggml_backend_cuda_get_device_count(); ++id) {
        ...
        const size_t packed_size = ggml_cuda_mxfp6_e2m3_tensor_size(ne0, nrows_copy, /*nplanes=*/1);
        char * packed = (char *) malloc(packed_size);
        GGML_ASSERT(packed != nullptr);
        tile_mxfp6_e2m3_blackwell_tensor * packed_tensor = (tile_mxfp6_e2m3_blackwell_tensor *) packed;
        memcpy(packed_tensor, data, MXFP6_HEADER_OFFSET);
        ggml_cuda_mxfp6_e2m3_patch_tensor_header(tensor, packed_tensor);
        GGML_ASSERT(ggml_cuda_set_mxfp6_tile_rows(tensor, row_low, nrows_copy, host_rows, packed_tensor->tiles));
    ...
    for (int id = 0; id < ggml_backend_cuda_get_device_count(); ++id) {
        ...
        const size_t original_size = split_size;
        // pad last row to a multiple of 512 elements to avoid out-of-bounds memory accesses
        if (ne0 % MATRIX_ROW_PADDING != 0) {
            split_size += ggml_row_size(tensor->type, MATRIX_ROW_PADDING - ne0 % MATRIX_ROW_PADDING);
        }

        GGML_UNUSED(split_size);
...
static void ggml_backend_cuda_split_buffer_get_tensor(ggml_backend_buffer_t buffer, const ggml_tensor * tensor, void * data, size_t offset, size_t size) {
    ...
    if (tensor->type == GGML_TYPE_NVFP4) {
        ...
        for (int id = 0; id < ggml_backend_cuda_get_device_count(); ++id) {
            ...
            const size_t packed_size = ggml_cuda_nvfp4_tensor_size(ne0, nrows_split, /*nplanes=*/1);
            char * packed = (char *) malloc(packed_size);
            GGML_ASSERT(packed != nullptr);
            ...
            ggml_cuda_unpack_tiles_nvfp4(ne0, nrows_split, packed_tensor->tiles, host_rows + row_low * row_size);
            free(packed);
        }

        memcpy(data, host_rows, logical_size);
        free(host_rows);
        return;
    }
    if (tensor->type == GGML_TYPE_MXFP6_E2M3) {
        ...
        for (int id = 0; id < ggml_backend_cuda_get_device_count(); ++id) {
            ...
            const tile_mxfp6_e2m3_blackwell_tensor * packed_tensor = (const tile_mxfp6_e2m3_blackwell_tensor *) packed;
            memcpy(host_rows, packed_tensor, MXFP6_HEADER_OFFSET);
            GGML_ASSERT(ggml_cuda_get_mxfp6_tile_rows(tensor, row_low, nrows_split, packed_tensor->tiles, host_rows + MXFP6_HEADER_OFFSET));
            free(packed);
            handled = true;
        }
...
static cudaError_t ggml_cuda_cpy_tensor_2d(
    ...
    if (type == GGML_TYPE_MXFP6_E2M3) {
        GGML_ABORT("MXFP6 CUDA tensors use tiled GGUF storage; compact copy fallback is unsupported");
    }
#endif // defined(BLACKWELL_MMA_AVAILABLE)
...
static void ggml_cuda_op_mul_mat(
    ...
    const bool src0_is_contiguous_raw = ggml_is_contiguous(src0);
    const bool src1_is_contiguous = ggml_is_contiguous(src1);
#if defined(BLACKWELL_MMA_AVAILABLE)
    const bool src0_is_contiguous = src0_is_contiguous_raw;
    const bool use_native_nvfp4 = src0->type == GGML_TYPE_NVFP4;
    const bool use_native_mxfp6 = src0_is_contiguous && src0->view_src == nullptr && src0->type == GGML_TYPE_MXFP6_E2M3 &&
        src0->ne[0] % QK_MXFP6_E2M3 == 0 &&
        blackwell_mma_available(ggml_cuda_info().devices[ggml_cuda_get_device()].cc);
    const int64_t src1_padded_col_size = GGML_PAD(ne10, MATRIX_ROW_PADDING);
    const bool use_native_nvfp4_mmq = use_native_nvfp4 && op == ggml_cuda_op_mul_mat_q;
    const bool use_native_mxfp6_mmq = use_native_mxfp6 && op == ggml_cuda_op_mul_mat_q;
    const size_t src1_ddq_nvfp4_row_size = ggml_cuda_nvfp4_blocks_per_row(src1_padded_col_size) * sizeof(block_nvfp4_mmq);
    const size_t src1_ddq_mxfp6_row_size = ggml_cuda_mxfp6_e2m3_blocks_per_row(src1_padded_col_size) * sizeof(tile_mxfp6_e2m3_mmq);
    const bool use_native_scaled_mmq = use_native_nvfp4_mmq;
    const ggml_tensor * scale_x_t = use_native_scaled_mmq ? ggml_cuda_mul_mat_input_scale(dst) : nullptr;
    ...
    const ggml_tensor * scale_x_src = use_native_scaled_mmq ? src0->src[1] : nullptr;
    const bool scale_x_in_header = use_native_scaled_mmq &&
        scale_x_src != nullptr && ggml_is_scalar(scale_x_src);
#else
    const bool src0_is_contiguous = src0_is_contiguous_raw;
    const bool use_native_nvfp4 = false;
    const bool use_native_mxfp6 = false;

    const int64_t src1_padded_col_size = GGML_PAD(ne10, MATRIX_ROW_PADDING);
    const bool use_native_nvfp4_mmq = false;
    const bool use_native_mxfp6_mmq = false;
    ...
    for (int id = 0; id < ggml_backend_cuda_get_device_count(); ++id) {
        ...
        if (!use_native_nvfp4 && !use_native_mxfp6 && ne00 % MATRIX_ROW_PADDING != 0 && ggml_is_quantized(src0->type) && ggml_backend_buffer_get_usage(src0->buffer) == GGML_BACKEND_BUFFER_USAGE_COMPUTE && src0->view_src == nullptr) {
        ...
        if (quantize_src1) {
            const size_t src1_ddq_row_size = use_native_nvfp4_mmq ?
                    src1_ddq_nvfp4_row_size :
                use_native_mxfp6_mmq ?
                    src1_ddq_mxfp6_row_size : src1_padded_col_size*q8_1_ts/q8_1_bs;
            size_t src_1_ddq_size = nrows1*src1_ddq_row_size;
            if (op == ggml_cuda_op_mul_mat_q) {
                src_1_ddq_size += get_mmq_x_max_host(dev[id].cc)*
                    (use_native_nvfp4_mmq ? sizeof(block_nvfp4_mmq) :
                     use_native_mxfp6_mmq ? sizeof(tile_mxfp6_e2m3_mmq) :
                     sizeof(block_q8_1_mmq));
            }
            dev[id].src1_ddq = dev[id].src1_ddq_alloc.alloc(ctx.pool(id), src_1_ddq_size);

            if (src1_on_device && src1_is_contiguous) {
                if (use_native_nvfp4_mmq) {
    ...
    for (int64_t src1_col_0 = 0; src1_col_0 < ne11; src1_col_0 += src1_col_stride) {
        ...
        for (int id = 0; id < ggml_backend_cuda_get_device_count(); ++id) {
            ...
            for (int64_t i0 = 0; i0 < ne13*ne12; ++i0) {
                ...
                } else if (quantize_src1 == (quantize_cuda_t) quantize_mmq_mxfp6_e2m3_cuda) {
                    src1_ddq_i_offset += src1_col_0 * sizeof(tile_mxfp6_e2m3_mmq);
                } else {
...
static bool ggml_cuda_should_fuse_mul_mat(const ggml_tensor * ffn_up,
    ...
    const auto nvfp4_scalar_scales = [](const ggml_tensor * t) {
        const ggml_tensor * w_s    = ggml_cuda_mul_mat_weight_scale(t);
        const ggml_tensor * w_in_s = ggml_cuda_mul_mat_input_scale(t);
        return t->op == GGML_OP_MUL_MAT && t->src[0]->type == GGML_TYPE_NVFP4 &&
               (w_s == nullptr || ggml_is_scalar(w_s)) && (w_in_s == nullptr || ggml_is_scalar(w_in_s));
    };
    if (((ggml_cuda_mul_mat_weight_scale(ffn_up)   || ggml_cuda_mul_mat_input_scale(ffn_up))   && !nvfp4_scalar_scales(ffn_up)) ||
        ((ggml_cuda_mul_mat_weight_scale(ffn_gate) || ggml_cuda_mul_mat_input_scale(ffn_gate)) && !nvfp4_scalar_scales(ffn_gate))) {
        return false;
    }
...
static bool ggml_cuda_should_fuse_mul_mat_vec_f(const ggml_tensor * tensor) {
    ...
    const bool nvfp4_scalar_scales = src0->type == GGML_TYPE_NVFP4 &&
        (w_s == nullptr || ggml_is_scalar(w_s)) && (w_in_s == nullptr || ggml_is_scalar(w_in_s));
    if ((w_s || w_in_s) && !nvfp4_scalar_scales) {
        return false;
    }
...
static void ggml_cuda_mul_mat(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    bool use_mul_mat_q     = ggml_is_quantized(src0->type) && !bad_padding_clear
        && src1->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32;
#if defined(BLACKWELL_MMA_AVAILABLE)
    const bool staged_mxfp_view = src0->type == GGML_TYPE_MXFP6_E2M3 &&
        src0->view_src != nullptr &&
        blackwell_mma_available(ggml_cuda_info().devices[ctx.device].cc);
#else
    const bool staged_mxfp_view = false;
#endif // defined(BLACKWELL_MMA_AVAILABLE)
    ...
    if (!split && use_mul_mat_vec_f) {
    ...
    } else if (!split && use_mul_mat_vec_q && !staged_mxfp_view) {
        ggml_cuda_mul_mat_vec_q(ctx, src0, src1, nullptr, dst);
    } else if (!split && use_mul_mat_q && !staged_mxfp_view) {
        ...
        quantize_cuda_t quantize = quantize_mmq_q8_1_cuda;
#if defined(BLACKWELL_MMA_AVAILABLE)
        if (src0->type == GGML_TYPE_NVFP4) {
            quantize = (quantize_cuda_t) quantize_mmq_nvfp4_cuda;
        }
#endif // defined(BLACKWELL_MMA_AVAILABLE)
        ggml_cuda_op_mul_mat(ctx, src0, src1, dst, ggml_cuda_op_mul_mat_q, quantize);
    } else {
...
static void ggml_cuda_mul_mat_id(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    for (int64_t i02 = 0; i02 < ne02; ++i02) {
        ...
        src0_slice.view_offs = src0->view_offs + i02*nb02;
    ...
    get_rows_cuda(dst_sorted.ptr, type_dst_sorted, ids_from_sorted, dst->data, dst->type,
        ne0, ne_get_rows, 1, 1, ne0*ts_dst_sorted, ne_get_rows*ne0*ts_dst_sorted, ne_get_rows*ne0*ts_dst_sorted,
        ne_get_rows, 1, 1, sizeof(int32_t), ne_get_rows*sizeof(int32_t), ne_get_rows*sizeof(int32_t),
        nb1, nb2, nb3, stream);
}
...
static bool ggml_backend_cuda_cpy_tensor_async(ggml_backend_t backend_src, ggml_backend_t backend_dst, const ggml_tensor * src, ggml_tensor * dst) {
    ...
    bool use_special_blackwell_layout = src->type == GGML_TYPE_NVFP4;
#if defined(BLACKWELL_MMA_AVAILABLE)
    use_special_blackwell_layout = use_special_blackwell_layout ||
        ggml_cuda_use_mxfp6_blackwell_layout(src, cuda_ctx_src->device);
#endif // defined(BLACKWELL_MMA_AVAILABLE)
    if (use_special_blackwell_layout) {
        size_t copy_size = ggml_nbytes(src);
#if defined(BLACKWELL_MMA_AVAILABLE)
        copy_size = src->type == GGML_TYPE_NVFP4 ?
                ...
                if (!disable_fusion) {
                    ...
                    for (ggml_op op : { GGML_OP_MUL_MAT, GGML_OP_MUL_MAT_ID }) {
                        ...
                        if (ggml_cuda_can_fuse(cgraph, i, { op, bias_op, op, bias_op, GGML_OP_GLU }, {})) {
                            ...
                            const ggml_tensor * ids  = up_n->op == GGML_OP_MUL_MAT_ID ? up_n->src[2] : nullptr;
                            ...
                            const ggml_tensor * ids  = up->op == GGML_OP_MUL_MAT_ID ? up->src[2] : nullptr;
                    ...
                    if (ggml_cuda_can_fuse(cgraph, i, { GGML_OP_SSM_CONV, GGML_OP_UNARY }, { GGML_UNARY_OP_SILU })) {
                        ggml_cuda_op_ssm_conv(*cuda_ctx, node, nullptr, cgraph->nodes[i+1]);
                        i++;
                        continue;
                    }

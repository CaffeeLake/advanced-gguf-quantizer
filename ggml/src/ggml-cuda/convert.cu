...
template <typename dst_t>
static __global__ void dequantize_tile_mxfp6_e2m3(const void * __restrict__ vx, dst_t * __restrict__ yy) {
    const int64_t i = blockIdx.x;
    const int tid = threadIdx.x;
    const compact_mxfp6_k32 & xb = ((const compact_mxfp6_k32 *) vx)[i];

#pragma unroll
    for (int frag = 0; frag < QK_MXFP6_E2M3_FRAGS; ++frag) {
        const uint8_t code = ggml_cuda_mxfp6_e2m3_get_code(xb, frag, tid);
        const float d = ggml_cuda_e8m0_to_fp32(xb.e[frag]);
        yy[i*QK_MXFP6_E2M3 + frag*QK_MXFP6_E2M3_SUB + tid] =
            ggml_cuda_cast<dst_t>(d * (float(kvalues_mxfp6_e2m3[code]) * (1.0f / 8.0f)));
    }
}

template <typename dst_t>
static void dequantize_row_mxfp6_e2m3_cuda(const void * vx, dst_t * y, const int64_t k, cudaStream_t stream) {
    GGML_ASSERT(k % QK_MXFP6_E2M3 == 0);
    const int nb = k / QK_MXFP6_E2M3;
    dequantize_tile_mxfp6_e2m3<<<nb, 32, 0, stream>>>(vx, y);
}

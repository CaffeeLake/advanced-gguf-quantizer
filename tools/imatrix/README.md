...
* `-t | --threads` and `-tb | --threads-batch` control CPU worker counts. The imatrix collector uses `--threads-batch` for host-side activation accumulation and PPL reduction.
* `-b | --batch-size` and `-ub | --ubatch-size` control prompt batching and physical micro-batches; tune these with VRAM headroom in mind.

Periodic checkpoint writes use temporary files and atomic replacement so an
interrupted run should not corrupt the last complete imatrix output.
...
```bash
# generate importance matrix using default filename (imatrix.gguf)
./llama-imatrix -m source-f16.gguf -f calibration-data.txt -t 22 -tb 22 -b 2048 -ub 1024

# use the imatrix to perform a Q4_K_M quantization
./llama-quantize --imatrix imatrix.gguf source-f16.gguf ./model-q4_k_m.gguf q4_k_m
```

```bash
# generate and save the imatrix using legacy format
./llama-imatrix -m source-f16.gguf -f calibration-data.txt --output-format dat -o imatrix-legacy-format.dat
```

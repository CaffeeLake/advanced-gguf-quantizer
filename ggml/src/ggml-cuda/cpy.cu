...
void ggml_cuda_cpy(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, ggml_tensor * src1) {
    ...
    if (src0->type == src1->type && contiguous_srcs) {
...
#endif // GGML_USE_MUSA && GGML_MUSA_MUDNN_COPY
        {
            ...
            } else if (src0->type == GGML_TYPE_MXFP6_E2M3 &&
                    blackwell_mma_available(ggml_cuda_info().devices[ggml_cuda_get_device()].cc)) {
                copy_size = ggml_cuda_mxfp6_e2m3_tensor_alloc_size(src0);
                GGML_ASSERT(copy_size == ggml_cuda_mxfp6_e2m3_tensor_alloc_size(src1));
            }

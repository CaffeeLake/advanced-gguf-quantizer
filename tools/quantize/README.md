...
```bash
# Start from an existing source GGUF, for example one produced by a separate
# upstream llama.cpp checkout.
...
```bash
# start a retained same-tree generation smoke test
./llama-completion -m /models/mymodel/ggml-model-Q4_K_M.gguf -p "You are a helpful assistant" -n 64
```

...
llama_context::llama_context(
    ...
    graph_reuse_disable = params.graph_reuse_disable;
...
const ggml_tensor * llama_context::get_logits_tensor_internal() const {
    if (!gf_res_prev) {
        return nullptr;
    }
    return gf_res_prev->get_logits();
}

ggml_backend_t llama_context::get_logits_backend_internal() const {
    const ggml_tensor * logits_tensor = get_logits_tensor_internal();
    if (logits_tensor == nullptr || !sched) {
        return nullptr;
    }
    return ggml_backend_sched_get_tensor_backend(sched.get(), const_cast<ggml_tensor *>(logits_tensor));
}
...
void llama_context::set_selector_gpu_kld_active(bool value) {
    selector_gpu_kld_active = value;
}
...
int llama_context::decode(const llama_batch & batch_inp) {
    ...
    do {
        ...
        // extract logits
        if (!selector_gpu_kld_active && logits.data && t_logits && n_outputs > 0 && needs_raw_logits(ubatch, sampling.samplers)) {
            ggml_backend_t backend_res = ggml_backend_sched_get_tensor_backend(sched.get(), t_logits);
            GGML_ASSERT(backend_res != nullptr);
            GGML_ASSERT(logits.data != nullptr);
...
const ggml_tensor * llama_internal_get_logits_tensor(const llama_context * ctx) {
    ...
    return ctx->get_logits_tensor_internal();
}

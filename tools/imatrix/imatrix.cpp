...
public:
    ...
    void request_save_imatrix(int32_t n_chunk = -1);
    void flush_saves() const;
    bool load_imatrix_legacy(const char * fname);
    bool load_imatrix(const char * file_name);
    const std::unordered_map<std::string, Stats> & get_mstats() const { return m_stats; }
private:
    IMatrixSnapshot snapshot_locked() const;
    void maybe_schedule_save_locked(int64_t count, int32_t chunk_size, std::vector<int32_t> & save_requests);

    std::unordered_map<std::string, Stats> m_stats;
    common_params                          m_params;
    mutable std::mutex                     m_mutex;
    mutable std::mutex                     m_save_mutex;
    mutable std::thread                    m_save_worker;
    std::vector<std::string>               m_datasets;
    int32_t                                m_last_chunk = 0;
};

static int imatrix_worker_count(const common_params & params) {
...
static void imatrix_parallel_for(const int64_t n, const int n_threads_requested, const Fn & fn) {
    const int n_threads = std::max<int64_t>(1, std::min<int64_t>(n_threads_requested, n));
    ...
    for (int ith = 1; ith < n_threads; ++ith) {
        const int64_t start = (n * ith) / n_threads;
        const int64_t end   = (n * (ith + 1)) / n_threads;
...
static std::string imatrix_tmp_path(const std::string & fname) {
    return fname + ".tmp";
}

static bool imatrix_replace_file(const std::string & tmp, const std::string & fname) {
#if defined(_WIN32)
    std::remove(fname.c_str());
#endif
    if (std::rename(tmp.c_str(), fname.c_str()) != 0) {
        const int err = errno;
        LOG_ERR("%s: failed to replace %s with %s: %s\n", __func__, fname.c_str(), tmp.c_str(), std::strerror(err));
...
static Stats collect_dense_imatrix_local(
    ...
    const int64_t n_cols = src1->ne[0];
    const int64_t n_mat  = src0->ne[2] * src0->ne[3];
    local.values.resize(n_cols * n_mat, 0.0f);
    local.counts.resize(1, ggml_nrows(src1) / n_mat);

    const int n_workers = imatrix_worker_count(params);
    imatrix_parallel_for(n_cols, n_workers, [&](const int64_t j_begin, const int64_t j_end) {
        for (int64_t i3 = 0; i3 < src1->ne[3]; ++i3) {
            for (int64_t i2 = 0; i2 < src1->ne[2]; ++i2) {
                const int64_t mat_id    = (i3 % src0->ne[3]) * src0->ne[2] + (i2 % src0->ne[2]);
                const int64_t mat_start = mat_id * n_cols;

                for (int64_t row = 0; row < src1->ne[1]; ++row) {
                    const float * x = (const float *) (data + row * src1->nb[1] + i2 * src1->nb[2] + i3 * src1->nb[3]);
                    for (int64_t j = j_begin; j < j_end; ++j) {
                        local.values[mat_start + j] += x[j] * x[j];
                    }
                }
            }
        }
...
static Stats collect_moe_imatrix_local(
        const struct ggml_tensor * src0,
        const struct ggml_tensor * src1,
    ...
    local.values.resize(n_cols * n_as, 0.0f);
    local.counts.resize(n_as, 0);

    for (int64_t idx = 0; idx < n_ids; ++idx) {
        for (int64_t row = 0; row < src1->ne[2]; ++row) {
            const int excur = *(const int32_t *) (ids_data + row * ids->nb[1] + idx * ids->nb[0]);
    ...
    imatrix_parallel_for(n_cols, n_workers, [&](const int64_t j_begin, const int64_t j_end) {
        for (int64_t idx = 0; idx < n_ids; ++idx) {
            for (int64_t row = 0; row < src1->ne[2]; ++row) {
                ...
                const int64_t e_start = excur * n_cols;
                for (int64_t j = j_begin; j < j_end; ++j) {
                    local.values[e_start + j] += x[j] * x[j];
                }
            }
        }
    });
...
bool IMatrixCollector::collect_imatrix(struct ggml_tensor * t, bool ask, void * user_data) {
    ...
    std::vector<char> src1_data;

    if (!is_host) {
        const size_t src1_nbytes = ggml_nbytes(src1);
        src1_data.resize(src1_nbytes);
        ggml_backend_tensor_get(src1, src1_data.data(), 0, src1_nbytes);
    }

    const char * data = is_host ? (const char *) src1->data : src1_data.data();
    GGML_ASSERT(src1->nb[0] == ggml_element_size(src1));

    std::vector<int32_t> save_requests;
    ...
    if (t->op == GGML_OP_MUL_MAT_ID) {
        ...
        std::vector<char> ids_data(ggml_nbytes(ids));
        ggml_backend_tensor_get(ids, ids_data.data(), 0, ggml_nbytes(ids));

        Stats local = collect_moe_imatrix_local(src0, src1, ids, data, ids_data.data(), m_params);
        ...
        for (size_t j = 0; j < local.values.size(); ++j) {
            e.values[j] += local.values[j];
        ...
        for (int64_t ex = 0; ex < n_as; ++ex) {
            e.counts[ex] += local.counts[ex];
            maybe_schedule_save_locked(e.counts[ex], chunk_size, save_requests);
        }
    } else {
        const int64_t n_mat = src0->ne[2] * src0->ne[3];
        Stats local = collect_dense_imatrix_local(src0, src1, data, m_params);
        ...
        for (size_t i = 0; i < e.counts.size(); ++i) {
            e.counts[i] += local.counts[0];
            maybe_schedule_save_locked(e.counts[i], chunk_size, save_requests);
        }
    }

    for (const int32_t n_chunk_save : save_requests) {
        request_save_imatrix(n_chunk_save);
    }

    return true;
}

IMatrixSnapshot IMatrixCollector::snapshot_locked() const {
    IMatrixSnapshot snapshot;
    snapshot.stats      = m_stats;
    snapshot.params     = m_params;
    snapshot.datasets   = m_datasets;
    snapshot.last_chunk = m_last_chunk;
    return snapshot;
}

void IMatrixCollector::maybe_schedule_save_locked(
        const int64_t count,
    ...
    const int32_t n_chunk = count / chunk_size;
    ...
    if (m_params.n_out_freq > 0 && (m_last_chunk % m_params.n_out_freq) / chunk_step == 0) {
        save_requests.push_back(-1);
    }
    if (m_params.n_save_freq > 0 && (m_last_chunk % m_params.n_save_freq) / chunk_step == 0) {
        save_requests.push_back(m_last_chunk);
    }
}

void IMatrixCollector::request_save_imatrix(int32_t n_chunk) {
    std::lock_guard<std::mutex> save_lock(m_save_mutex);
    if (m_save_worker.joinable()) {
        m_save_worker.join();
    }
    m_save_worker = std::thread([this, n_chunk]() {
        this->save_imatrix(n_chunk);
    });
}

void IMatrixCollector::flush_saves() const {
    std::lock_guard<std::mutex> save_lock(m_save_mutex);
    if (m_save_worker.joinable()) {
        m_save_worker.join();
    }
...
void IMatrixCollector::save_imatrix_legacy(int32_t ncall) const {
    ...
    auto fname = snapshot.params.out_file;
    ...
    for (const auto & kv : snapshot.stats) {
    ...
    if (to_store.size() < snapshot.stats.size()) {
        LOG_WRN("%s: storing only %zu out of %zu entries\n", __func__, to_store.size(), snapshot.stats.size());
    }

    // deterministic tensor name order
    std::sort(to_store.begin(), to_store.end());

    const int32_t chunk_size = snapshot.params.n_ctx / snapshot.params.n_parallel;

    const std::string tmp_fname = imatrix_tmp_path(fname);
    std::ofstream out(tmp_fname, std::ios::binary);
    out.write((const char *) &n_entries, sizeof(n_entries));
    for (const auto & name : to_store) {
        const auto & stat = snapshot.stats.at(name);
    ...
    out.write((const char *) &snapshot.last_chunk, sizeof(snapshot.last_chunk));

    // Write the input filename at the end of the file to later on specify it in quantize
    {
        const char * dataset_file = snapshot.params.prompt_file.c_str();
        int32_t len = snapshot.params.prompt_file.size();
        // When there is no prompt but there were other imatrix files loaded, use the last dataset
        if (snapshot.params.prompt_file.empty() && !snapshot.datasets.empty()) {
            const std::string & dataset_str = snapshot.datasets[snapshot.datasets.size() - 1];
    ...
    out.close();
    if (!out) {
        LOG_ERR("%s: failed writing temporary imatrix %s\n", __func__, tmp_fname.c_str());
        std::remove(tmp_fname.c_str());
        return;
    }
    imatrix_replace_file(tmp_fname, fname);

    LOGV(1, "\n");
    LOG_DBGV(1, "%s: stored collected data after %d chunks in %s\n", __func__, snapshot.last_chunk, fname.c_str());
}
...
void IMatrixCollector::save_imatrix(int32_t n_chunk) const {
    ...
    auto fname = snapshot.params.out_file;
    int8_t use_legacy_format = snapshot.params.imat_dat;
...

    {
        ...
        datasets.reserve(snapshot.datasets.size() + 1);
        for (size_t i = 0; i < snapshot.datasets.size(); ++i) {
            datasets.push_back(snapshot.datasets[i].c_str());
        }
        if (!snapshot.params.prompt_file.empty()) {
            datasets.push_back(snapshot.params.prompt_file.c_str());
        }
        ...
        gguf_set_val_u32(ctx_gguf, LLM_KV_IMATRIX_CHUNK_COUNT, snapshot.last_chunk);
        gguf_set_val_u32(ctx_gguf, LLM_KV_IMATRIX_CHUNK_SIZE, snapshot.params.n_ctx / snapshot.params.n_parallel);
    }
    ...
    const std::string tmp_fname = imatrix_tmp_path(fname);
    gguf_write_to_file(ctx_gguf, tmp_fname.c_str(), false);
    imatrix_replace_file(tmp_fname, fname);
...
static void process_logits(
    ...
    auto compute = [&result_mutex, &counter, &nll, &nll2, logit_history, prob_history, n_vocab, logits, tokens, n_token] () {
        double local_nll  = 0;
        double local_nll2 = 0;
        while (true) {
...
static bool compute_imatrix(llama_context * ctx, const common_params & params, const int32_t n_ctx) {
    ...
    std::vector<std::thread> workers(std::max(0, imatrix_worker_count(params) - 1));
...
int main(int argc, char ** argv) {
    ...
    LOG_INF("%s: batch = %d, ubatch = %d, threads = %d, threads_batch/collector = %d\n",
            __func__, params.n_batch, params.n_ubatch,
            params.cpuparams.n_threads, imatrix_worker_count(params));
